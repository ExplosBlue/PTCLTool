...
#define RG_ETC1_UNUSED(x) ((void)x)
...
{
    const uint limit = diff ? 32 : 16; RG_ETC1_UNUSED(limit);
...
static void dither_block_555(color_quad_u8* dest, const color_quad_u8* block)
{
    ...
    memset(reinterpret_cast<void*>(dest), 0xFF, sizeof(color_quad_u8)*16);
